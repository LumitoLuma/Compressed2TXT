@(echo off% <#%) &color 0f &title Compressed2TXT v6.1 mod - Files/Folders "Send to" menu .bat ascii encoder by AveYo. Mod by Lumito
...
# Choice text - &x is optional hotkey toggle      # Choice value       # Default:1
  ...
  $c += '&7  No console output (in the new file) '; $v += 'no-output'  ; $d += 0
...
# Show Choices dialog snippet - outputs $result with indexes like '1,2,4'
  ...
  $result = Choices $all $def 'Compressed2TXT v6.1 mod' 14
...
# Generate text decoding header - compact self-expanding batch file for bundled ascii encoded cab archive of target files
  $HEADER  = "@echo off & color 0f`r`n"
# Choice 7: No console output (in the new file)
  if ($choices -eq 'no-output') {
    $HEADER += 'set "0=%~f0" & powershell -nop -c $f=[IO.File]::ReadAllText($env:0)-split'':bat2file\:.*'';iex($f[1]); X 1 >nul 2>nul'
    $HEADER += "`r`n@exit/b`r`n`r`n:bat2file: Compressed2TXT v6.1`r`n"
  } else {
